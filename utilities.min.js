...
export function groupBy(lookupFn,sourceArray){return sourceArray?sourceArray.reduce((groupObj,obj)=>{const group=lookupFn(obj);return{...groupObj,[group]:[...(groupObj[group]??[]),obj]};},{}):groupBy.bind(null,lookupFn);}
...
export function loopRange(max,min=0){return function move(cur,dir){return((cur+dir+max-min)%max)+min;};}
...
export function cloneObject(obj){if(obj===null||typeof obj!=='object'||'__isActiveClone'in obj)
...
export function enumerate(source=[],options={}){const{numericValues=false,constatntProperties=false}=options;return(Array.isArray(source)?source:Object.keys(source)).reduce((obj,key,index)=>{const propertyName=constatntProperties?toGlobal(key):key;return{...obj,[propertyName]:numericValues?index:key};},{});function toGlobal(_propertyName){const hasLowerCase=/[a-z]/.test(_propertyName);return hasLowerCase?_propertyName.replace(/([A-Z/])/g,'_$1').toUpperCase():_propertyName;}}